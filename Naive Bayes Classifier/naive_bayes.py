...
class NaiveBayes():
    ...
    def predict(self, filename:str):
        ...
        for word in words:
            log_prob_ham += np.log((self.word_counts_ham.get(word, 0) + 1) / (self.num_train_hams + 2))
            log_prob_spam += np.log((self.word_counts_spam.get(word, 0) + 1) / (self.num_train_spams + 2))
